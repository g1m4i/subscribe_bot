...
from src.domain.common.events.base import MiddlewareType, NextMiddlewareType
...
class Observer:
    ...
    async def notify(self, events: List[Event], data: Dict[str, Any]) -> None:
        for event in events:
            handlers = self.handlers.get(type(event), [])
            for handler in handlers:
                wrapped_handler = self._wrap_middleware(
                    self.middlewares,
                    handler,
                )
    ...
    def middleware(self, middleware: MiddlewareType) -> MiddlewareType:
